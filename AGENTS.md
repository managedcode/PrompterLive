...
- Teleprompter reader background-media controls must not use picture-in-picture/card-in-card, file-only, recording-camera, or abstract stacked-layer icons; use an obvious framed media/background glyph so the affordance reads as choosing a background source that may be live camera, local video file, or URL/video media.
...
- The TPS cue matrix fixture must keep the same one-cue isolation contract as the README screenshots: `starter-tps-cue-matrix.tps` needs one block per visual treatment, one normal sentence per block, and one central cue-bearing word or short phrase per block; do not group unrelated TPS tags in the same example card. Keep a single authoritative copy of this fixture in `src/PrompterOne.Shared/Library/SeedData/` and have tests consume that one file instead of duplicating it under `tests/TestData/Scripts/`.
...
- TPS speed cue letter-spacing must stay visually restrained at README screenshot scale: slow and xslow should broaden the cue word enough to communicate pace, but must not stretch letters into distracting decorative gaps or make the word feel wider than its sentence can comfortably support.
...
- TPS legato, crescendo, stress, underline, pronunciation, phonetic, and other decorative reader marks must render as smooth vector-quality strokes at README screenshot scale; do not use stretched glyphs, CSS gradients, raster images, or pixel-grid tricks that create jagged, stair-stepped, or pixelated marks.
...
- Editor file transfer must preserve multiple explicit formats: importing should support native TPS/Markdown/plain text and document formats such as DOCX through the picker path, while exporting must keep TPS, Markdown, plain text, and DOCX as separate choices instead of replacing or hiding existing formats.
...
- dropdown and popover components must collision-check the viewport when opened and choose direction and horizontal offset so the menu stays visible; do not rely on a fixed down-opening CSS position for controls near screen edges
