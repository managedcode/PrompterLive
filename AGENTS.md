...
- Script graph views must prioritize readable hierarchy over decorative circular placement: highlight the main/root document node, group TPS structure, source lines, people, themes, archetypes, and cue metadata by graph value/kind instead of by label text, expose node and edge hover tooltips with source detail, provide zoom/fit/layout controls, and avoid noisy duplicated node-list strips under the graph.
