...
public sealed class TeleprompterControlRailTests : BunitContext
{
    ...
    private const string RecordingMicrophoneTooltipText = "Microphone captured in the recording";
    private const string RecordingModeTooltipText = "Choose whether to record video + audio or audio only";
    ...
    public void TeleprompterPage_RendersSeparateBackgroundCameraAndRecordingControls()
    {
        ...
        cut.WaitForAssertion(() =>
        {
            ...
            Assert.Contains("Video + audio", recordingMode.TextContent, StringComparison.Ordinal);
            Assert.Contains("Camera", cameraSelect.TextContent, StringComparison.Ordinal);
            Assert.Contains("Mic", microphoneSelect.TextContent, StringComparison.Ordinal);
            Assert.Contains(RecordingModeTooltipText, recordingMode.GetAttribute("aria-label") ?? string.Empty, StringComparison.Ordinal);
            Assert.Contains(RecordingCameraTooltipText, cameraSelect.GetAttribute("aria-label") ?? string.Empty, StringComparison.Ordinal);
            Assert.Contains(RecordingMicrophoneTooltipText, microphoneSelect.GetAttribute("aria-label") ?? string.Empty, StringComparison.Ordinal);
            ...
            Assert.Equal(RecordingModeTooltipText, cut.FindByTestId(UiTestIds.Tooltip.Surface(UiTestIds.Teleprompter.RecordingModeSelect)).TextContent.Trim());
