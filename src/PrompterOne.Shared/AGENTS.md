...
- Editor workspace authoring modes must be explicit: `Raw` is the canonical TPS source editor with tags, and `Editor` is the visual WYSIWYG-style editable authoring mode. Do not label the visual authoring mode as `Rendered` or generic cards when the user needs to edit without knowing Markdown or TPS syntax.
- Editor styled-text mode must expose visible cue affordances directly on the editable blocks and segments, including tone, pace, pause, energy, volume, delivery, pronunciation, and similar TPS intent, so non-technical authors can understand and revise the script without reading raw tags.
